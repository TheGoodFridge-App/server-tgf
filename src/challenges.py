...
def update_user_challenges():
    ...
    labels = []
    ref = db.collection('products_associated_with_labels').where(
        brand[0], u'array_contains', brand)
...
def get_user_challenges():
    ...
    try:
        ref = db.collection(u'users').document(str(email)).collection(
            'challenges').document('challenges')
        data = ref.get().to_dict()
...
def add_challenges(email, challenge_data):
    ...
    ongoing_and_completed_challenges = challenge_arr + \
        challenge_data['history']
    new_challenges_needed = 3 - len(challenge_arr)

    undone_challenges = [
        challenge for challenge in all_challenges if challenge not in ongoing_and_completed_challenges]
    challenges = dict(challenge_data['challenges'])

    for i in range(new_challenges_needed):
        new_challenge = undone_challenges[random.randint(
            0, len(undone_challenges))]
    ...
    ref = db.collection(u'users').document(email).collection(
        'challenges').document('challenges')
    ref.set({
